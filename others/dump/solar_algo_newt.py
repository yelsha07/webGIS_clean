...
def IRENA_monthly_ghi(final_solar_data): #IRENA
    monthly_ghi_data.extend([tuple(entry[i] for entry in final_solar_data) for i in range(2, 14)])
    return monthly_ghi_data

#MONTHLY ENERGY YIELD
def NSRDB_monthly_energy_yield(sum_months, pixel_num, area=9, af=0.7, eta=0.2): #NSRDB
    """
...
def IRENA_monthly_energy_yield(monthly_ghi_data, pixel_num, area=9, af=0.7, eta=0.2): #IRENA
    """
    Compute Monthly Energy Yield (MEY) for each month using total GHI.
    """
    mey_list_irena = []
    for month_ghi in monthly_ghi_data:
        ...
        MEY = (ghi_sum * area * pixel_num * af * eta)/1000 # Convert to MWh
...
def IRENA_capacity (valid_points, points_num, power_density=1000, area=9):
    cap_irena = (area * power_density * points_num)/1000000
    return cap_irena
#-------------------------------------------------------------------------------------------------------
